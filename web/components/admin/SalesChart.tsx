	...
	return (
		<div>
			<svg viewBox={`0 0 ${width} ${height}`} className="w-full" role="img" aria-label="Monthly sales chart">
				<line x1={0} y1={baselineY} x2={width} y2={baselineY} stroke="var(--color-border)" strokeWidth={1} />
			...
			{data.length < 3 && (
				<p className="mt-2 text-center text-xs text-text-subtle">
					Sales history builds up month over month — check back soon to see the full trend.
				</p>
			)}
		</div>
	);
